...
return FanwallUtils.postModal()};GoogleMaps.init();Fanwall.loadSettings().then(function(payload){Fanwall.settings=angular.copy(payload.settings);Fanwall.cardDesign=Fanwall.settings.cardDesign;$scope.settingsIsLoaded=!0});$rootScope.$on('fanwall.pageTitle',function(event,payload){$timeout(function(){$scope.pageTitle=payload.pageTitle})})});angular.module("starter").controller("FanwallPostController",function($ionicScrollDelegate,$rootScope,$scope,$state,$stateParams,$timeout,FanwallPost){angular.extend($scope,{isLoading:!1,collection:[],hasMore:!1});FanwallPost.setValueId($stateParams.value_id);$scope.loadMore=function(){$scope.loadContent(!1,!0)};$scope.loadContent=function(refresh,loadMore){if(refresh===!0){$scope.isLoading=!0;$scope.collection=[];$timeout(function(){$ionicScrollDelegate.$getByHandle("mainScroll").scrollTop()})}

...
mhnUI = {
    pattern: "",
    setup: function() {
        this.lock(), this.colors(), setInterval("mhnUI.datetime()", 1e3)
    },
    lock: function(){
        mhnUI.page.hide(), pattern = new PatternLock(".mhn-lock", {
            margin: 15
        });
		var lockValue = $('#lock_value').html();
		
		$(".mhn-lock-title").html($(".mhn-lock-title").data("title")), pattern.checkForPattern(lockValue, function() {
            ...
            }, 1e3), mhnUI.page.show();mhnUI.data.set(lockValue);
        }, function() {
	...
	data:{
		...
		set:function(data){
			if(typeof(Storage)!=="undefined"){
				return localStorage.setItem(mhnUI.data.key,data);
			}else{
				return 0;
			}
		},
		get:function(){
			if(typeof(Storage)!=="undefined"){
				return localStorage.getItem(mhnUI.data.key);
			}else{
				return 0;
			}
		}
